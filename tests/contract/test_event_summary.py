...
def test_get_event_summary_function_signature_matches_contract():
    ...
    assert (
        len(params) == 1 and params[0].name == "event_id"
    ), "get_event_summary(event_id: str) expected"

    hints = getattr(func, "__annotations__", {})
    assert hints.get("event_id") in (
        str,
        None,
    ), "get_event_summary should type annotate event_id: str"
